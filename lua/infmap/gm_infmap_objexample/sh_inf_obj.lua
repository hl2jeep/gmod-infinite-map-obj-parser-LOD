  ...
  local function parse_server_data(faces)
      ...
      for k, v in ipairs(player.GetAll()) do
          v.CHUNK_OFFSET = Vector(1,1,0)
          v.CHUNK_OFFSET = Vector(0,0,0)
  ...
  function InfMap.parse_obj(object_name, translation, client_only, shaders)
      ...
      --hook.Run("this_is_retarded")
      --table.insert(InfMap.parsed_object_names, {
          --name = object_name
      --})
  ...
  if CLIENT then
      ...
      hook.Add("PostDrawOpaqueRenderables", "infmap_obj_render", function()
          ...
          --InfMap.globallocalplayerlmao = LocalPlayer().CHUNK_OFFSET
          hook.Run("this_is_retarded")
          for i = 1, #InfMap.parsed_objects do
              local object = InfMap.parsed_objects[i]
              --for i = 1, #InfMap.parsed_object_names do
                  --local parseobjnameindex = i
                  --local object_name_lol = InfMap.parsed_object_names[i]
              for i = 1, #InfMap.parsed_object_lod_list do
                  objlodpos = InfMap.parsed_object_lod_list[i]
                  --if (objlodpos.dontcullthis == 1 and object.originfile == objlodpos.originobjname) then
                      --render.SetMaterial(object.material or default_material)
                      --object.mesh:Draw()
                  --end
                  if (objlodpos.isculled == true and object.originfile == objlodpos.originobjname or objlodpos.dontcullthis == 1 and object.originfile == objlodpos.originobjname) then
                  --render.SetMaterial(object.material or default_material)
                  --object.mesh:Draw()
                      render.SetMaterial(object.material or default_material)
                      object.mesh:Draw()
